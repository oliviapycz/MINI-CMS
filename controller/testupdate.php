...
<html>
  <head>
    ...
    <body>
      ...
      <main>
        <nav class="nav">
          <ul>
        <?php
            $reponse = $bdd->query('SELECT country FROM article ORDER BY ID DESC LIMIT 0, 8');
    ...
    <article class="article">
      ...
      <?php 	$newreponse = $bdd->query('SELECT * FROM article
        ');
      		while ($donnees = $newreponse->fetch()) {
      ?>
